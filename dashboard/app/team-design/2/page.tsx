...
// Design 2 — "Orbital + Living Background"
// Emil skill: custom cubic-bezier, stagger, scale(0.95) entries, panel slide-in
// Taste skill: animated orb BG, double-bezel panel, no generic patterns, dimming of unrelated nodes
  ...
  return (
    <g transform={`translate(${x},${y})`} onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}
      ...
      <g style={{transform:`scale(${scale})`,transformOrigin:"0 0",transition:"transform 0.28s cubic-bezier(0.32,0.72,0,1)"}}>
        ...
        {agent.has_live_data&&<circle r={r+28} fill="none" stroke={agent.color} strokeWidth="1" opacity="0" className="d2-pulse-ring"/>}
        ...
        {agent.has_live_data&&<circle cx={r-5} cy={-(r-5)} r="4" fill="#10B981" stroke="#080C10" strokeWidth="1.5" className="d2-live-dot"/>}
      </g>
  ...
  return (
    <g>
      ...
      <path d={d} fill="none" stroke={active?color:`rgba(${rgb},0.2)`} strokeWidth={active?2.2:0.8}
        strokeDasharray="5 9" opacity={active?1:0.45} className="d2-signal"
  ...
  return (
    <div className="space-y-2">
      ...
      {Object.keys(report).filter(k=>k!=="generated_at"&&k!=="error").slice(0,8).map((key,i)=>{
        ...
        return (
          <div key={key} className="rounded-xl p-3 d2-stagger" style={{background:`rgba(${rgb},0.05)`,border:`1px solid rgba(${rgb},0.12)`,animationDelay:`${i*40}ms`}}>
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes d2-orb-a{0%,100%{transform:translate(0,0) scale(1)}45%{transform:translate(55px,-38px) scale(1.1)}80%{transform:translate(-25px,18px) scale(0.95)}}
        @keyframes d2-orb-b{0%,100%{transform:translate(0,0) scale(1)}40%{transform:translate(-65px,45px) scale(1.08)}75%{transform:translate(35px,-25px) scale(0.93)}}
        @keyframes d2-orb-c{0%,100%{transform:translate(0,0)}55%{transform:translate(28px,55px)}}
      ...
      <div className="min-h-[100dvh] flex flex-col pt-16 relative overflow-hidden" style={{background:"#080C10"}}>
        ...
        <div className="relative z-10 px-8 pt-8 pb-4 flex items-end justify-between">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-3 text-[10px] font-mono tracking-[0.18em]"
              style={{background:"rgba(14,165,233,0.07)",border:"1px solid rgba(14,165,233,0.13)",color:"#F5A623"}}>
        ...
        {/* Main */}
        <div className="relative z-10 flex-1 flex flex-col lg:flex-row gap-0 px-4 pb-8 min-h-0">
          ...
          <div className="w-full lg:w-[355px] shrink-0 py-4 pr-2">
            {selected ? (
              <div key={selected.id} className="d2-panel-in flex flex-col rounded-[1.6rem] p-1.5"
                style={{background:"rgba(255,255,255,0.035)",border:"1px solid rgba(255,255,255,0.07)",height:"calc(100vh - 200px)",boxShadow:"0 0 100px rgba(0,0,0,0.6)"}}>
                        ...
                        {selected.has_live_data&&(
                          <div className="space-y-2">
                            {[{label:"FOCUS",val:selected.current_focus},{label:"VIEW",val:selected.market_view},{label:"ACTIVITY",val:selected.recent_activity}].map(({label,val},i)=>(
                              <div key={label} className="rounded-xl p-3 d2-stagger" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",animationDelay:`${i*55}ms`}}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {agents.map((a,i)=>(
                    <button key={a.id} onClick={()=>{setSelected(a);setTab("overview");}}
                      className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-left cursor-pointer d2-stagger"
